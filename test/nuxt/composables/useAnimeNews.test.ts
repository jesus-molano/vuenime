...
const TestComponent = defineComponent({
  ...
  setup(props) {
    ...
    const { news, isLoading, error, refresh } = useAnimeNews(id)
    return { news, isLoading, error, refresh }
  },
  render() {
    return h('div', { class: 'test-component' }, [
      h('span', { id: 'news-count' }, String(this.news.length)),
...
describe('useAnimeNews (Nuxt)', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  describe('composable structure', () => {
    it('should return news as array', async () => {
      const wrapper = await mountSuspended(TestComponent, {
        props: { animeId: '1' },
      })
      await nextTick()

      expect(wrapper.vm.news).toBeDefined()
