...
v-else
        class="grid grid-cols-1 gap-4 pb-20 sm:grid-cols-2 sm:gap-4 md:grid-cols-3 md:gap-5 md:pb-8 lg:grid-cols-4 xl:grid-cols-5 xl:gap-6">
        <AnimeCard v-for="anime in searchResults.data" :key="anime.mal_id" :anime="anime" />
      </div>
    </UContainer>
  </div>
</template>
...
const { data: searchResults, status } = useFetch<AnimeListResponse>(
  '/api/jikan/anime',
  {
