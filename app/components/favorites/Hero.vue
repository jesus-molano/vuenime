<template>
  <section class="relative overflow-hidden bg-rp-base pb-6 pt-20 sm:pb-8 md:pb-12 md:pt-28">
    ...
    <UContainer class="relative z-10 px-4 sm:px-6">
      <div class="flex flex-col items-center gap-6 md:flex-row md:gap-12 lg:gap-16">
        <!-- Image side -->
        <div class="relative shrink-0">
          <!-- Glow effect -->
          <div class="absolute -inset-4 rounded-full bg-rp-love/15 blur-2xl" />
          <div class="absolute -inset-2 rounded-full bg-rp-iris/10 blur-xl" />

          <NuxtImg
            src="/images/fav.webp"
            alt=""
            aria-hidden="true"
            class="relative h-28 w-auto object-contain drop-shadow-[0_0_25px_rgba(235,111,146,0.25)] sm:h-36 md:h-44 lg:h-52"
            loading="eager"
          />
        </div>
        ...
        <div class="flex-1 text-center md:text-left">
          <!-- Title -->
          <h1 class="text-2xl font-bold tracking-tight text-rp-text sm:text-3xl md:text-4xl lg:text-5xl">
            {{ $t('favorites.title') }}
          </h1>

          <!-- Subtitle -->
          <p class="mt-2 max-w-lg text-sm text-rp-subtle sm:text-base md:mt-3 md:text-lg">
          ...
          <ClientOnly>
            ...
            <div
              v-if="isSyncing"
              class="mt-4 flex flex-wrap items-center justify-center gap-2 md:mt-5 md:justify-start md:gap-3"
            >
              <div class="h-8 w-36 animate-pulse rounded-full bg-rp-surface/80 ring-1 ring-rp-overlay/30" />
            ...
            <!-- SSR Skeleton fallback - simple pills -->
            <template #fallback>
              <div class="mt-4 flex flex-wrap items-center justify-center gap-2 md:mt-5 md:justify-start md:gap-3">
                <div class="h-8 w-36 animate-pulse rounded-full bg-rp-surface/80 ring-1 ring-rp-overlay/30" />
                <div class="h-8 w-28 animate-pulse rounded-full bg-rp-surface/80 ring-1 ring-rp-overlay/30" />
              </div>
            </template>
