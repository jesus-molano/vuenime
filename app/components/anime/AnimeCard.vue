<template>
  ...
  <article
    ...
    :style="{ viewTransitionName: isNavigating ? `anime-card-${anime.mal_id}` : '' }" tabindex="0" role="article"
    :aria-labelledby="`anime-title-${anime.mal_id}`" @click="navigateToDetail" @keydown.enter="navigateToDetail">
    ...
    <div class="relative hidden aspect-3/4 overflow-hidden sm:block">
      ...
      <div
        class="absolute inset-0 hidden items-center justify-center bg-rp-iris/80 opacity-0 backdrop-blur-sm transition-opacity duration-300 group-hover:opacity-100 md:flex">
